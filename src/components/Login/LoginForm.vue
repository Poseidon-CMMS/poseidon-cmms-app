<template>
  <div class="mt-6">
    <form id="login-form" @submit.prevent="onSubmit(!v$.$invalid)" class="">
      <Card style="margin: auto auto" class="lg:w-4">
        ...
        <template #content>
          <div class="grid p-fluid mt-2">
            ...
            <div class="col-12" v-if="!!error">
              <Message severity="error" @close="onErrorClose"
                >Error de login, revise sus credenciales o el estado del
                backend</Message
              >
            </div>
          </div>
  ...
  methods: {
    async onSubmit(isFormValid) {
      ...
      try {
        ...
        if (result.data.authenticate.item.type == "admin") {
          this.$router.push("/");
        } else {
